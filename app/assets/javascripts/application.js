...
window.GOVUKPrototypeKit.documentReady(() => {
  ...
  // ---- Save confirmation banner (GOV.UK notification banner, success) ----
  // Shown above a section after Save changes. Only one on the page at a time.
  let bannerCount = 0
 
  const removeSuccessBanner = () => {
    const existing = document.querySelector('.dwp-save-banner')
    if (existing) existing.remove()
  }
 
  const showSuccessBanner = (group) => {
    removeSuccessBanner()
 
    const heading = group.querySelector('.dwp-section-heading h2')
    const sectionName = heading ? heading.textContent.trim() : 'Section'
 
    bannerCount += 1
    const titleId = 'dwp-save-banner-title-' + bannerCount
 
    const banner = document.createElement('div')
    banner.className = 'govuk-notification-banner govuk-notification-banner--success dwp-save-banner'
    ...
    message.className = 'govuk-notification-banner__heading'
    message.textContent = sectionName + ' changes have been saved.'
  ...
  document.querySelectorAll('[data-editable-group]').forEach((group) => {
    ...
    editLink.addEventListener('click', (e) => {
      ...
      // Editing again clears any previous save confirmation
      removeSuccessBanner()
      ...
      const exit = (save) => {
        ...
        // Confirm the save; on cancel put focus back on the Edit link
        if (save) {
          showSuccessBanner(group)
        } else {
          editLink.focus()
        }
      }
